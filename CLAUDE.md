...
**PHASE 1 COMPLETE (Modules 1–9). Case study written — the top-level [README.md](README.md) IS the case study (front page = story + measured results + reproduce steps).** Closing experiment: full **all-Sonnet agent** on the 50-q golden set, **Sonnet-judged** ([evals/results/2026-06-08-agent-sonnet-50q.json](evals/results/2026-06-08-agent-sonnet-50q.json)). Vs Haiku agent (completeness both Sonnet-judged → clean): **completeness 4.24 → 4.62 (+0.38)**, gains scaling with reasoning difficulty — synthesis **+1.13** (coverage flat 52→56% → pure generation lift), contradiction **+0.78**, multi-hop **+0.67**; easy cats +0.1–0.2. Honest trade-off: **out-of-scope refusal regressed 86→71%** (Sonnet leaked 2/7 Caesar-adjacent OOS — caesar-salad, augustus-successor); faithfulness reads 4.40 (NOT clean vs Haiku 4.72 — judge-mismatch + OOS leaks); cost 1.6× ($0.040 vs $0.026/q). **This retroactively justifies the M9 router** (Sonnet wins = categories a router escalates; losses = the cheap categories it keeps on Haiku) → shipping the routing policy is the natural Module 10 move. Routing NOT wired into the agent (M9 classifier-accuracy eval stands alone); dropped per user steer (best quality = all-Sonnet). **Module 10 = separate repo, slightly different stack (clean restart).** Prior module logs below.

**Module: 9 — Production Patterns COMPLETE (full log + measured impact: [notes/module-9-production-patterns.md](notes/module-9-production-patterns.md)). Operational layer on the agent, every pattern EVAL-GATED at near-zero spend: (1) Prompt caching — rolling transcript + system/tools cache in `runTools` (`cacheSystem`/`runAgent({cache})`), ~2× lower per-Q cost. (2) Cross-provider fallback — `lib/fallback.ts` `createFallbackClient` chain Claude→local Qwen (`runAgent({fallback})`); break-key demo: 401→full loop on Qwen at $0. (3) Rate limiting — `lib/rate-limit.ts` token-bucket+semaphore (proactive batch pacing; SDKs still own 429 backoff). (4) Model routing — `lib/route.ts` Haiku classifier (escalation-biased) + `evals/route-eval.ts`; vs gold categories: 74% acc, only 3/26 DANGEROUS misroutes (all contradiction Qs invisible at question-level) → low quality risk; ship/no-ship A/B parked for M10. (5) Guardrails — `lib/guardrails.ts` `validateInput`/`validateOutput` wired into `runAgent` (default on); `evals/guardrails-eval.ts`: 0 false positives, 7/7 attacks, caught the 1 known leak (q-045). (6) Prompt versioning — Langfuse `registerPrompt`/`getManagedPrompt` + in-code fallback, `runAgent({useManagedPrompt})` opt-in, `register-prompt.ts`; live v1/v2+label rollback verified. NO runtime fact-check (judge already measures grounding). Open→M10: routing agent A/B, fallback `llmLabel` static. Prior: Module 8 MCP server core done + smoke-tested (Claude-Code-wired via `.mcp.json`); demo screenshot (Claude Desktop/Inspector) still outstanding. Module 7 agent COMPLETE — beats single-shot both models (Claude comp 3.40→4.04, refusal 92→98%; qwen 3.02→3.38).**
