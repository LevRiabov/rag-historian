...
**Module: 8 — MCP Server Design IN PROGRESS. `roman-research/mcp-server/{index.ts,README.md}` + repo-root `.mcp.json` (wires it into Claude Code). `@modelcontextprotocol/sdk` high-level `McpServer` over **stdio**; reuses the SAME canonical stack as the agent (`contextual-v1` + rerank, frozen `RETRIEVAL` const) — second front door to `retrieve`, NOT a new retriever. Exposes all THREE primitives deliberately (the lesson — a tools-only/GraphQL-passthrough server collapses these): TOOLS (model-controlled) `search_roman_corpus`, `search_roman_source`, `read_roman_chunk`, `list_roman_sources`, `cite_passage`; RESOURCES (app-controlled) `roman://sources` + templated `roman://source/{slug}` (4 books enumerable via list cb); PROMPTS (user-controlled) `research_topic`, `summarize_event`. Smoke-tested end-to-end via SDK stdio client: handshake + all 3 lists + DB-backed list + full retrieval path (embeddings+rerank) OK. **stdout = JSON-RPC channel; all logs to stderr (footgun).** `.mcp.json` activates on next Claude Code session start in-repo. TODO: connect in Claude Desktop + MCP Inspector for the demo screenshot (deliverable). Prior: Module 7 agent COMPLETE — agent beats single-shot for both models (Claude comp 3.40→4.04, refusal 92→98%; qwen comp 3.02→3.38); open→Module 10: contradiction completeness (generation-bound), qwen over-search/weaker refusal. Module 7 findings: [notes/module-7-agent.md](notes/module-7-agent.md).**
